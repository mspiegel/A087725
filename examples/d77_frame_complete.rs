...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let refs_path: PathBuf = std::env::args()
        .nth(1)
        .ok_or("usage: d77_frame_complete REFS_FILE [MIN_H] [CACHE]")?
        .into();
    ...
    for (idx, (top, arr)) in frames.iter().enumerate() {
        ...
        loop {
            ...
            if !next_permutation(&mut bottom) {
                break;
            }
        }
        ...
        for (r, d) in results {
            *hist.entry(d).or_insert(0) += 1;
            if d >= 76 && all_finds.insert(r, d).is_none() {
                ...
                if rr != r {
                    all_finds.entry(rr).or_insert(d);
                }
                if d == 77 {
                    fnew77 += 1;
                }
        ...
        if fnew77 > 0 || (idx % 20 == 0) {
            println!(
                "frame {:>3}/{:<3} cand={} new_d77={} (total d77={})",
                idx + 1,
                frames.len(),
                cands.len(),
                fnew77,
    ...
    println!(
        "\n=== done in {:.1?} : {} frames, {} candidates verified ===",
        t0.elapsed(),
        frames.len(),
        total_cand
    );
