...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let ranks_path: PathBuf = std::env::args()
        .nth(1)
        .ok_or("usage: verify_depths RANKS_FILE [OUT_PAIRS] [LIMIT]")?
        .into();
    let out_pairs: Option<PathBuf> = std::env::args()
        .nth(2)
    ...
    let mut ranks: Vec<u64> = fs::read_to_string(&ranks_path)?
        .lines()
    ...
    let ckpt = out_pairs
        .as_ref()
        .map(|p| Mutex::new(BufWriter::new(fs::File::create(p).unwrap())));
    let results: Vec<(u64, u8)> = ranks
        .par_iter()
        .map(|&r| {
            ...
            if c % 5000 == 0 {
                ...
                eprintln!(
                    "  {c}/{n}  {:.1} solves/s  elapsed {:.0}s  (d>=75 finds: {})",
                    c as f64 / el,
                    el,
    ...
    for &(r, d) in &finds {
        ...
        print!(
            "FIND d={d} rank={r} (reflect={rr}) blank@{}: ",
            s.blank_pos()
        );
        for (j, &t) in s.0.iter().enumerate() {
            if t == 0 {
