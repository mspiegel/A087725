...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let ranks_path: PathBuf = std::env::args().nth(1).ok_or("usage: show_ranks RANKS_FILE [CACHE]")?.into();
    let cache_path: Option<PathBuf> = std::env::args().nth(2).map(Into::into);

    let ranks: Vec<u64> = fs::read_to_string(&ranks_path)?
        .lines()
        .filter_map(|l| { let s = l.trim(); if s.is_empty() || s.starts_with('#') { None } else { s.parse::<u64>().ok() } })
        .collect();

    let cache = match &cache_path { Some(p) => Some(cache::load(p)?), None => None };
    ...
    for r in &ranks {
        ...
        println!("rank={r:>14}  d={}  h={hv}  blank@{}  refl={rr}{}",
                 d.map(|x| x.to_string()).unwrap_or("?".into()),
                 s.blank_pos(),
                 if cache.is_some() { if refl_cached { " (refl cached)" } else { " (refl MISSING)" } } else { "" });
        for row in 0..4 {
            print!("    ");
            for col in 0..4 { let t = s.0[row*4+col]; if t==0 { print!(" __"); } else { print!(" {t:>2}"); } }
            println!();
        }
        println!();
    }
