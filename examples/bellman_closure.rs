...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let neighbors = |r: u64| -> Vec<u64> {
        let s = unrank(r);
        let b = s.blank_pos();
        State::legal_moves_at(b)
            .iter()
            .map(|m| rank(&s.apply_at(m, b).0))
            .collect()
    };
    ...
    for &r in &seeds {
        for nr in neighbors(r) {
            if !c.contains_key(&nr) && inwork.insert(nr) {
                stack.push(nr);
            }
        }
    }
    ...
    while let Some(r) = stack.pop() {
        ...
        if processed % 2_000_000 == 0 {
            eprintln!(
                "  processed {processed}, stack {}, new so far {}",
                stack.len(),
