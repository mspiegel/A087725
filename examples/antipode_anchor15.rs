...
fn main() {
    ...
    if !store.layer(79).is_empty() && !store.layer(78).is_empty() {
        let a79: Vec<i32> = store
            .layer(79)
            .iter()
            .map(|&r| anchor_est(&unrank(r)))
            .collect();
        let a78: Vec<i32> = store
            .layer(78)
            .iter()
            .map(|&r| anchor_est(&unrank(r)))
            .collect();
        ...
        println!(
            ...
            max78,
            if min79 > max78 {
                "SEPARABLE (a threshold splits 79 from 78)"
            } else {
                "OVERLAP (no anchor threshold separates 79 from 78)"
            },
        );
