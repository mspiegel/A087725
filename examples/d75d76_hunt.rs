...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let grids_path: PathBuf = std::env::args()
        .nth(1)
        .ok_or("usage: d75d76_hunt PART_GRIDS MODEL K OUT_PREFIX [CACHE]")?
        .into();
    ...
    for b in &grids {
        let mut key = [[0u8; 4]; 4];
        for (q, cells) in QUAD.iter().enumerate() {
    ...
    let per: Vec<(Vec<Cand>, Vec<Cand>)> = partitions
        .par_iter()
        .map(|key| {
            let q: [Vec<[u8; 4]>; 4] = [
                perms
                    .iter()
                    .map(|p| [key[0][p[0]], key[0][p[1]], key[0][p[2]], key[0][p[3]]])
                    .collect(),
                perms
                    .iter()
                    .map(|p| [key[1][p[0]], key[1][p[1]], key[1][p[2]], key[1][p[3]]])
                    .collect(),
                perms
                    .iter()
                    .map(|p| [key[2][p[0]], key[2][p[1]], key[2][p[2]], key[2][p[3]]])
                    .collect(),
                perms
                    .iter()
                    .map(|p| [key[3][p[0]], key[3][p[1]], key[3][p[2]], key[3][p[3]]])
                    .collect(),
            ];
                ...
                for b in &q[1] {
                    for cc in &q[2] {
                        for d in &q[3] {
                            ...
                            if hv < 59 {
                                continue;
                            } // d76 even>=60, d75 odd>=59
                            feats[0] = s.blank_pos() as f64;
                            for i in 0..16 {
                                feats[i + 1] = g[i] as f64;
                            }
                            let sc = model.score(&feats) as f32;
                            if hv % 2 == 0 {
                                even.push(Cand { score: sc, rank: r });
                            } else {
                                odd.push(Cand { score: sc, rank: r });
                            }
                        }
                    }
                }
    ...
    w.flush()?;
    eprintln!(
        "wrote {} merged ranks (score-interleaved) -> {pm}",
        merged.len()
    );
    Ok(())
}
