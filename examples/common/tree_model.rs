...
struct Node { is_leaf: bool, feat: u32, thr: f64, left: u32, right: u32, val: f64 }
struct Tree { nodes: Vec<Node> }

pub struct TreeModel { baseline: f64, trees: Vec<Tree>, pub nfeat: usize }
    ...
    pub fn load(path: &Path) -> std::io::Result<TreeModel> {
        ...
        for line in &mut lines {
            ...
            match toks[0] {
                ...
                "tree" => {
                    if let Some(t) = cur.take() { trees.push(t); }
                    remaining = toks[1].parse().unwrap();
                    cur = Some(Tree { nodes: Vec::with_capacity(remaining) });
                }
                _ => {
                    // node line
                    let n = Node {
        ...
        if let Some(t) = cur.take() { trees.push(t); }
        Ok(TreeModel { baseline, trees, nfeat })
    }
    ...
    pub fn raw(&self, x: &[f64]) -> f64 {
        ...
        for t in &self.trees {
            ...
            loop {
                ...
                if nd.is_leaf { acc += nd.val; break; }
                i = if x[nd.feat as usize] <= nd.thr { nd.left } else { nd.right };
            }
        }
        acc
    }
