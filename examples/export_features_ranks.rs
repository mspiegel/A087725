...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let pairs_path: PathBuf = std::env::args()
        .nth(1)
        .ok_or("usage: export_features_ranks PAIRS.tsv OUT.tsv")?
        .into();
    let out_path: PathBuf = std::env::args().nth(2).ok_or("need OUT.tsv")?.into();

    let f = std::fs::File::open(&pairs_path)?;
    let pairs: Vec<(u64, u8)> = std::io::BufReader::new(f)
        .lines()
        .filter_map(|l| {
            let l = l.ok()?;
    ...
    let rows: Vec<String> = pairs
        ...
        .map(|&(r, d)| {
            ...
            for (i, &v) in s.0.iter().enumerate() {
                ...
                let (cr, cc) = (i / 4, i % 4);
                man +=
                    (gr as i32 - cr as i32).unsigned_abs() + (gc as i32 - cc as i32).unsigned_abs();
            }
