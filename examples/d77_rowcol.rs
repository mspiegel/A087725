...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (&r, &d) in &c {
        ...
        let s = unrank(r);
        let b = s.blank_pos();
        let is_max = !State::legal_moves_at(b).iter().any(|m| {
            let (ns, _) = s.apply_at(m, b);
        ...
        n += 1;
        for cell in 0..16 {
            cell_count[s.0[cell] as usize][cell] += 1;
        }
    }
    ...
    for t in 1..=15u8 {
        let mut cells: Vec<(usize, u32)> = (0..16)
            .filter(|&cl| cell_count[t as usize][cl] > 0)
            .map(|cl| (cl, cell_count[t as usize][cl]))
            .collect();
        ...
        for &(cl, cnt) in &cells {
            *rows.entry(cl / 4).or_insert(0) += cnt;
            *cols.entry(cl % 4).or_insert(0) += cnt;
        }
        let rstr: String = rows
            .iter()
            .map(|(r, c)| format!("{r}:{}%", 100 * c / n))
            .collect::<Vec<_>>()
            .join(",");
        let cstr: String = cols
            .iter()
            .map(|(cl, c)| format!("{cl}:{}%", 100 * c / n))
            .collect::<Vec<_>>()
            .join(",");
        let cellstr: String = cells
            .iter()
            .take(8)
            .map(|&(cl, cnt)| format!("{cl}({cnt})"))
            .collect::<Vec<_>>()
            .join(" ");
    ...
    for (&r, &d) in &c {
        ...
        blanks.insert(b as usize);
        blank_count[b as usize] += 1;
    }
    println!(
        "\nblank cells: {:?}",
        blanks
            .iter()
            .map(|&b| format!("{b}({})", blank_count[b]))
            .collect::<Vec<_>>()
    );
    println!(
        "\nrough product of per-tile distinct-cell counts: e^{:.1} = {:.2e}",
        prod_log,
        prod_log.exp()
    );
    Ok(())
