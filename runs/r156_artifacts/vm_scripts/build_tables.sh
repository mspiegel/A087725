...
echo "=== [$(date -u +%H:%M:%S)] stage 0: cargo build --release --features sha"
cargo build --release --features sha 2>&1 | tail -3

echo "=== [$(date -u +%H:%M:%S)] stage 1: build_wd24"
target/release/build_wd24 --out data/wd24.bin --verify-sha data/wd24.bin.sha256 2>&1 | tail -5

echo "=== [$(date -u +%H:%M:%S)] stage 2: build_cwd_table"
target/release/build_cwd_table 2>&1 | tail -8

echo "=== [$(date -u +%H:%M:%S)] stage 3: build_cwd_artifacts all"
target/release/build_cwd_artifacts all 2>&1 | tail -10
...
for g in a b c; do
  ...
  target/release/build_pdb24 --zero-aware --tiles $TILES \
    --out data/pdb24_k8_$g.zbin --verify-sha data/pdb24_k8_$g.sha256 2>&1 | tail -6
done
