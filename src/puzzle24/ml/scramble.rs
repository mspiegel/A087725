...
//! uniform in `[1, k_max]` (a fixed range, not an advancing curriculum). We reuse
//! the repo's inline-xorshift convention rather than pulling in the `rand` crate,
//! matching how the existing 24-puzzle tests generate randomness (and the
//! `random_walk` helper in `examples/ladder24.rs`).
...
/// legal blank moves, never immediately undoing the previous move. Returns the
/// resulting state and the actual `k` taken. The result is always solvable.
...
/// Uniform random walk of **exactly** `k` steps from GOAL (undo-pruned). Used by
/// the eval harness to build a holdout spread over a target depth range.
pub fn scramble_exact(rng: &mut Rng, k: u32) -> State {
    let choose = |_s: &State, blank: u8, rng: &mut Rng| -> Move {
        let legal: Vec<Move> = State::legal_moves_at(blank).iter().collect();
        legal[rng.gen_range(0, (legal.len() - 1) as u32) as usize]
    };
...
/// rollout plugs into. Undo-pruning is applied *around* `choose`.
