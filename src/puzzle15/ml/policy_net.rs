...
//!
//! Maps a one-hot board (`256`) to 4 move logits (order = `Move::ALL`). During a
...
impl Module for PolicyNet {
    /// `x: [batch, 256]` → `[batch, 4]` raw move logits.
...
///
/// Illegal moves — and the optional `banned` move (used to forbid immediately
/// undoing the previous step, matching how `scramble`/DeepCubeA generate boards)
...
pub fn sample_move(
    ...
    let x = encode_batch(std::slice::from_ref(s), device)?; // [1, 256]
...
mod tests {
    ...
    fn only_legal_moves_are_sampled() {
        // GOAL's blank is at the bottom-right corner (cell 15): only Up and Left
        // are legal. Over many samples we must never see Down or Right.
        let dev = Device::Cpu;
        let vm = VarMap::new();
        let net = PolicyNet::new(VarBuilder::from_varmap(&vm, DType::F32, &dev), 32).unwrap();
    ...
    fn banned_move_is_never_sampled() {
        // From an interior state (blank in the middle) all 4 moves are legal;
        // banning one must ensure it's never chosen.
        let dev = Device::Cpu;
        let vm = VarMap::new();
        let net = PolicyNet::new(VarBuilder::from_varmap(&vm, DType::F32, &dev), 32).unwrap();
        let mut rng = Rng::new(77);
        // Move GOAL's blank to an interior cell (5) so all 4 moves are legal.
        let mut s = GOAL;
