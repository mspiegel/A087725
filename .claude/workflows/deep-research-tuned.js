export const meta = {
  name: 'deep-research-tuned',
  description: 'Deep research harness — cost-tuned variant. Sonnet for search/fetch/verify; Opus for scope/synthesize. Smaller fetch and verify caps.',
  whenToUse: 'When the user wants a deep, multi-source, fact-checked research report on any topic and is cost-sensitive. BEFORE invoking, check if the question is specific enough to research directly — if underspecified (e.g., "what car to buy" without budget/use-case/region), ask 2-3 clarifying questions to narrow scope. Then pass the refined question as args, weaving the answers in.',
  phases: [{"title":"Scope","detail":"Decompose question (from args) into 5 search angles (Opus)"},{"title":"Search","detail":"5 parallel WebSearch agents, one per angle (Sonnet)"},{"title":"Fetch","detail":"URL-dedup, fetch top 10 sources, extract falsifiable claims (Sonnet)"},{"title":"Verify","detail":"3-vote adversarial verification per claim, top 12 claims (Sonnet)"},{"title":"Synthesize","detail":"Merge semantic dupes, rank by confidence, cite sources (Opus)"}],
}

// Tuned variant of deep-research:
//   - Sonnet on search/fetch/verify (~3x cheaper per token vs Opus)
//   - Opus on scope/synthesize (where reasoning quality matters most)
//   - MAX_FETCH 15 → 10
//   - MAX_VERIFY_CLAIMS 25 → 12
// Verify is still 3 votes; the adversarial signal is the core value of the workflow.
// Expected agent count at cap: 1 + 5 + 10 + 36 + 1 = 53 (vs 97 in the original).

const VOTES_PER_CLAIM = 3
const REFUTATIONS_REQUIRED = 2
const MAX_FETCH = 10
const MAX_VERIFY_CLAIMS = 12
const CHEAP_MODEL = "claude-sonnet-4-6"
...
// ─── Phase 0: Scope — decompose question into search angles (Opus, inherited) ───
phase("Scope")
const QUESTION = (typeof args === "string" && args.trim()) || ""
if (!QUESTION) {
  return { error: "No research question provided. Pass it as args: Workflow({name: 'deep-research-tuned', args: '<question>'})." }
}
...
// search/fetch/verify all run on Sonnet — same web-grep + skeptical-read work,
// ~3x cheaper per token than Opus.
